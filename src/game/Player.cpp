...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder *holder )
{
    ...
    //"health, power1, power2, power3, power4, power5, exploredZones, equipmentCache, ammoId, actionBars, faction  FROM characters WHERE guid = '%u'", GUID_LOPART(m_guid));
...
bool Player::BuyItemFromVendor(ObjectGuid vendorGuid, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    */
    ...
    else if (IsEquipmentPos(bag, slot))
    {
        ...
        if (IsTraitor() && pProto->Class == ITEM_CLASS_JUNK && pProto->SubClass == ITEM_SUBCLASS_JUNK_MOUNT)
        {
